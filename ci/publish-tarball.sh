...
(
  ...
  scripts/cargo-install-all.sh stable "${RELEASE_BASENAME}" $1

  tar cvf "${TARBALL_BASENAME}"-$TARGET.tar "${RELEASE_BASENAME}"
  bzip2 "${TARBALL_BASENAME}"-$TARGET.tar
  cp "${RELEASE_BASENAME}"/bin/exzo-install-init exzo-install-init-$TARGET
...
for file in "${TARBALL_BASENAME}"-$TARGET.tar.bz2 "${TARBALL_BASENAME}"-$TARGET.yml exzo-install-init-"$TARGET"* $MAYBE_TARBALLS; do
