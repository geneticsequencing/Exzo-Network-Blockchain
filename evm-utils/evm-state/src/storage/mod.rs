...
impl<D: ExzoDBCommon> Clone for Storage<D> {
    fn clone(&self) -> Self {
        Self {
...
impl<D: ExzoDBCommon> std::fmt::Debug for Storage<D> {
...
static SECONDARY_MODE_PATH_SUFFIX: &str = "exzo-secondary";
...
pub trait ExzoDBCommon: DBAccess + std::fmt::Debug + Sized {
...
impl ExzoDBCommon for rocksdb::DBWithThreadMode<rocksdb::SingleThreaded> {
...
impl ExzoDBCommon for OptimisticTransactionDB {
    fn flush(&self) -> Result<(), rocksdb::Error> {
        self.flush()
    }
...
pub struct DbWithClose<D: ExzoDBCommon>(D);
...
impl<D: ExzoDBCommon> std::fmt::Debug for DbWithClose<D> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
