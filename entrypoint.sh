...
stake_account_exist() {
    exzo --keypair $datadir/identity.json --url $rpc_url stake-account $datadir/stake-account.json
}
...
run_solana_validator() {
    ...
    rpc_url=$(exzo-gossip rpc-url --timeout 180 --entrypoint $entrypoint) # get rpc url
    if [ $? -ne 0 ]; then
    ...
    case "$NETWORK" in
        ...
        "devnet" | "development")
            if ! vote_account_exist; then
                exzo-keygen new --no-passphrase -so $datadir/identity.json #try to generate identity
                exzo-keygen new --no-passphrase -so $datadir/vote-account.json #try to generate vote account
                exzo --keypair /config/faucet.json --url $rpc_url transfer $datadir/identity.json $(($MIN_VALIDATOR_STAKE + $MIN_RENT_FEE))
                exzo --keypair $datadir/identity.json --url $rpc_url create-vote-account $datadir/vote-account.json $datadir/identity.json
            fi
            
            if ! stake_account_exist; then
                # TODO: Airdrop tokens if not enough
                vote_account=$(exzo address --keypair $datadir/vote-account.json)
                exzo-keygen new --no-passphrase -so $datadir/stake-account.json
                exzo --keypair $datadir/identity.json --url $rpc_url create-stake-account $datadir/stake-account.json $MIN_VALIDATOR_STAKE
                stake_account=$(exzo address --keypair $datadir/stake-account.json)
                exzo --keypair $datadir/identity.json --url $rpc_url delegate-stake --force $stake_account $vote_account
            fi
        ;;
    esac
...
generate_first_node() {
    exzo-keygen new --no-passphrase -fso $DATADIR/faucet.json
    exzo-keygen new --no-passphrase -so $DATADIR/identity.json
    exzo-keygen new --no-passphrase -so $DATADIR/vote-account.json
    exzo-keygen new --no-passphrase -so $DATADIR/stake-account.json
    ...
    exzo-genesis --max-genesis-archive-unpacked-size 1073741824 --enable-warmup-epochs --bootstrap-validator $DATADIR/identity.json $DATADIR/vote-account.json $DATADIR/stake-account.json --bpf-program TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA BPFLoader2111111111111111111111111111111111 spl_token-3.1.0.so --bpf-program Memo1UhkJRfHyvLMcVucJwxXeuD728EqVDDwQDxFMNo BPFLoader1111111111111111111111111111111111 spl_memo-1.0.0.so --bpf-program MemoSq4gqABAXKb96qnH8TysNcWxMyWCqXgDLGmfcHr BPFLoader2111111111111111111111111111111111 spl_memo-3.0.0.so --bpf-program ATokenGPvbdGVxr1b2hvZbsiqW5xWH25efTNsLJA8knL BPFLoader2111111111111111111111111111111111 spl_associated-token-account-1.0.1.so --bpf-program Feat1YXHhH6t1juaWF74WLcfv4XoNocjXA6sPWHNgAse BPFLoader2111111111111111111111111111111111 spl_feature-proposal-1.0.0.so --ledger $DATADIR --faucet-pubkey $DATADIR/faucet.json --faucet-lamports $FAUCET_LAMPORTS --hashes-per-tick auto --cluster-type $NETWORK
}
