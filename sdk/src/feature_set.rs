...
lazy_static! {
    ...
    pub static ref FEATURE_NAMES_BEFORE_MAINNET: HashMap<Pubkey, &'static str> = [
        ...
        (full_inflation::devnet_and_testnet_exzo_mainnet::id(), "full inflation on devnet and testnet"),
    ...
    pub static ref FEATURE_NAMES: HashMap<Pubkey, &'static str> = FEATURE_NAMES_BEFORE_MAINNET.iter().map(|(k, v)| (*k, *v)).chain(
        [
            ...
            // Exzo features
            (exzo::hardfork_pack::id(), "EVMblockhashes sysvar history, roothashes calculation. Apply old (reconfigure_native_token, unlock_switch_vote)."),
            (exzo::evm_cross_execution::id(), "EVM cross execution."),
            (exzo::native_swap_in_evm_history::id(), "Native swap in evm history."),
            (exzo::evm_new_error_handling::id(), "EVM new error handling."),
            (exzo::unsigned_tx_fix::id(), "Authorized transaction hash fixed."),
            (exzo::ignore_reset_on_cleared::id(), "Don't reset evm_swap address balance, when it already swapped, to avoid empty blocks."),
            (exzo::free_ownership_require_signer::id(), "Free ownership require signer."),
            (exzo::burn_fee::id(), "Burn fee during transaction execution."),
            (exzo::clear_logs_on_error::id(), "Clear logs from receipt if transaction is failed or reverted."),
            (exzo::disable_durable_nonce::id(), "Disable durable nonce."),
            (exzo::evm_new_precompiles::id(), "Evm new precomplies pack."),
            (exzo::evm_instruction_borsh_serialization::id(), "Support for Borsh serialization for EVM instructions."),
            (exzo::accept_zero_gas_price_with_native_fee::id(), "Accept evm transactions with native fee and zero gas price."),
            (exzo::clear_logs_on_native_error::id(), "Clear evm logs from receipt if native transaction is failed."),
            /*************** ADD NEW FEATURES HERE ***************/
        ]
    ).collect();
...
impl FeatureSet {
    ...
    pub fn full_inflation_features_enabled(&self) -> HashSet<Pubkey> {
        ...
        if self.is_active(&full_inflation::devnet_and_testnet_exzo_mainnet::id()) {
            hash_set.insert(full_inflation::devnet_and_testnet_exzo_mainnet::id());
        }
        hash_set
    }
...
mod test {
    ...
    fn test_full_inflation_features_enabled_devnet_and_testnet() {
        ...
        feature_set
            .active
            .insert(full_inflation::devnet_and_testnet_exzo_mainnet::id(), 42);
        assert_eq!(
            feature_set.full_inflation_features_enabled(),
            [full_inflation::devnet_and_testnet_exzo_mainnet::id()]
                .iter()
                .cloned()
                .collect()
        );
