...
impl TryFrom<&[u8]> for ExzoAccountType {
    type Error = ParseError;

    fn try_from(data: &[u8]) -> Result<Self, Self::Error> {
        if data.len() == ACCOUNT_LEN {
            VAccountInfo::try_from_slice(data).map(ExzoAccountType::Account)
        } else {
            VAccountStorage::deserialize_stream_array(data).map(ExzoAccountType::Storage)
        }
