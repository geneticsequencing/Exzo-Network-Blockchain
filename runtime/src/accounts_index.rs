...
use exzo_account_program::{VAccountStorage, ExzoAccountType};
...
pub enum IndexKey {
    ...
    ExzoAccountStorage(Pubkey),
    ExzoAccountOwner(Pubkey),
    ExzoAccountOperational(Pubkey),
    ExzoRelyingOwner(Pubkey),
}
...
impl AccountIndex {
    pub fn name(&self) -> &str {
        match self {
            ...
            AccountIndex::ExzoAccountStorage => "Exzo Account Storage",
            AccountIndex::ExzoAccountOwner => "Exzo Account Owner",
            AccountIndex::ExzoAccountOperational => "Exzo Account Operational",
            AccountIndex::ExzoRelyingOwner => "Exzo Relying Owner",
        }
    }
}
...
pub struct AccountsIndex<T: IndexValue> {
    ...
    // Exzo Indices
    exzo_account_storage_index: SecondaryIndex<DashMapSecondaryIndexEntry>,
    exzo_account_owner_index: SecondaryIndex<DashMapSecondaryIndexEntry>,
    exzo_account_operational_index: SecondaryIndex<DashMapSecondaryIndexEntry>,
    exzo_relying_party_owner_index: SecondaryIndex<DashMapSecondaryIndexEntry>,
...
impl<T: IndexValue> AccountsIndex<T> {
    ...
    pub fn new(config: Option<AccountsIndexConfig>) -> Self {
        ...
        Self {
            ...
            //Exzo indexes
            exzo_account_storage_index: SecondaryIndex::<DashMapSecondaryIndexEntry>::new("exzo_account_storage_index"),
            exzo_account_owner_index: SecondaryIndex::<DashMapSecondaryIndexEntry>::new("exzo_account_owner_index"),
            exzo_account_operational_index: SecondaryIndex::<DashMapSecondaryIndexEntry>::new("exzo_account_operational_index"),
            exzo_relying_party_owner_index: SecondaryIndex::<DashMapSecondaryIndexEntry>::new("exzo_relying_party_owner_index"),
        }
    }
        ...
        R: RangeBounds<Pubkey> + std::fmt::Debug,
    {
        ...
        */
        match scan_type {
            ...
            ScanTypes::Indexed(IndexKey::ExzoAccountStorage(va_storage_key)) => self
                .do_scan_secondary_index(
                    ancestors,
                    func,
                    &self.exzo_account_storage_index,
                    &va_storage_key,
                    Some(max_root),
                    config,
                ),
            ScanTypes::Indexed(IndexKey::ExzoAccountOwner(va_owner_key)) => self
                .do_scan_secondary_index(
                    ancestors,
                    func,
                    &self.exzo_account_owner_index,
                    &va_owner_key,
                    Some(max_root),
                    config,
                ),
            ScanTypes::Indexed(IndexKey::ExzoAccountOperational(va_operational_key)) => self
                .do_scan_secondary_index(
                    ancestors,
                    func,
                    &self.exzo_account_operational_index,
                    &va_operational_key,
                    Some(max_root),
                    config,
                ),
            ScanTypes::Indexed(IndexKey::ExzoRelyingOwner(va_owner_key)) => self
                .do_scan_secondary_index(
                    ancestors,
                    func,
                    &self.exzo_relying_party_owner_index,
                    &va_owner_key,
                    Some(max_root),
                    config,
                ),
    ...
    pub(crate) fn update_secondary_indexes(
        ...
        // Exzo account
        if *account_owner == exzo_account_program::id() {
            match ExzoAccountType::try_from(account_data) {
                Ok(ExzoAccountType::Account(account_info)) => {
                    if account_indexes.contains(&AccountIndex::ExzoAccountStorage) {
                        let storage = account_info.find_storage_key(pubkey);
                        self.exzo_account_storage_index.insert(&storage, pubkey);
                    }

                    if account_indexes.contains(&AccountIndex::ExzoAccountOwner) {
                        for owner in account_info.owners {
                            if owner != Pubkey::default() {
                                self.exzo_account_owner_index.insert(&owner, pubkey);
                            }
                        }
                    }
                }
                Ok(ExzoAccountType::Storage(VAccountStorage { operationals })) => {
                    if account_indexes.contains(&AccountIndex::ExzoAccountOperational) {
                ...
                Err(err) => log::warn!("Unable to parse Exzo Account: {:?}", err),
            }
        }

        // Exzo relying party data
        if *account_owner == exzo_relying_party_program::id() {
            match RelyingPartyData::try_from(account_data) {
                Ok(acc) => {
                    if account_indexes.contains(&AccountIndex::ExzoRelyingOwner) {
                        self.exzo_relying_party_owner_index
